import React, { Component } from 'react';

import HomeFeedItem from '../HomeFeedItem';
import H from '../type-elements/H';

class HomeFeed extends Component {
  render() {
    return (
      <div id="HomeFeed">
        <HomeFeedItem
          postTitle="How I Take and Edit Photos on My Phone"
          volumeNumber="70"
          publishedDate="Jan 19 2017"
          postLink="/how-to-take-and-edit-photos-on-your-phone" />
        <HomeFeedItem
          postTitle="Here's My Illustration Process"
          volumeNumber="69"
          publishedDate="Jan 12 2017"
          postLink="/here-is-my-illustration-process" />
        <HomeFeedItem
          postTitle="My 2017"
          volumeNumber="68"
          publishedDate="Jan 5 2017"
          postLink="/2017" />
        <HomeFeedItem
          postTitle="Here’s a Giant List of Design Inspiration for You to Use"
          volumeNumber="67"
          publishedDate="Dec 27 2017"
          postLink="/giant-list-of-design-inspiration-to-use" />
        <HomeFeedItem
          postTitle="A Global Rule for Your Work"
          volumeNumber="66"
          publishedDate="Dec 15 2017"
          postLink="/a-global-rule-for-your-work" />
        <HomeFeedItem
          postTitle="Tips to Make Better Design Moodboards"
          volumeNumber="65"
          publishedDate="Dec 8 2017"
          postLink="/tips-to-make-better-design-moodboards" />
        <HomeFeedItem
          postTitle="Questions to Ask Employers Hiring Remotely"
          volumeNumber="64"
          publishedDate="Dec 1 2017"
          postLink="/questions-to-ask-employers-hiring-remotely" />
        <HomeFeedItem
          postTitle="Making the Most of Your Time Off"
          volumeNumber="63"
          publishedDate="Nov 24 2017"
          postLink="/making-the-most-of-your-time-off" />
        <HomeFeedItem
          postTitle="How to Come Up with Writing Topics"
          volumeNumber="62"
          publishedDate="Nov 17 2017"
          postLink="/how-to-come-up-with-writing-topics" />
        <HomeFeedItem
          postTitle="A Peek Into my Illustration Process"
          volumeNumber="61"
          publishedDate="Nov 10 2017"
          postLink="/peek-into-my-illustration-process" />
        <HomeFeedItem
          postTitle="Removing Choice from the Productivity Equation"
          volumeNumber="60"
          publishedDate="Nov 3 2017"
          postLink="/removing-choice-from-productivity-equation" />
        <HomeFeedItem
          postTitle="How to Work on Personal Projects after a Long Full-Time Job"
          volumeNumber="59"
          publishedDate="Oct 27 2017"
          postLink="/how-to-work-after-a-full-time-job" />
        <HomeFeedItem
          postTitle="The Mistake I Make that Kills My Productivity"
          volumeNumber="58"
          publishedDate="Oct 20 2017"
          postLink="/mistake-i-make-that-kills-my-productivity" />
        <HomeFeedItem
          postTitle="Hyper Light Drifter Guidance Design Review and Analysis"
          volumeNumber="57"
          publishedDate="Oct 13 2017"
          postLink="/hyper-light-drifter-design-review" />
        <HomeFeedItem
          postTitle="The Ebb and Flow of the Design Process"
          volumeNumber="56"
          publishedDate="Oct 6 2017"
          postLink="/ebb-and-flow-of-design-process" />
        <HomeFeedItem
          postTitle="How I Market Myself — Designer or Developer?"
          volumeNumber="55"
          publishedDate="Sep 29 2017"
          postLink="/marketing-yourself-as-designer-or-developer" />
        <HomeFeedItem
          postTitle="Additional Before Necessary Patterns in Tech Products"
          volumeNumber="54"
          publishedDate="Sep 22 2017"
          postLink="/additional-before-necessary-patterns" />
        <HomeFeedItem
          postTitle="The Real Reason I Love Apple Events"
          volumeNumber="53"
          publishedDate="Sep 15 2017"
          postLink="/real-reason-i-love-apple-events" />
        <HomeFeedItem
          postTitle="One Year of Weekly Writing, Time for Change"
          volumeNumber="52"
          publishedDate="Sep 9 2017"
          postLink="/one-year-of-weekly-writing" />
        <HomeFeedItem
          postTitle="The Job Search Needs Change"
          volumeNumber="51"
          publishedDate="Sep 1 2017"
          postLink="/the-job-search-needs-change" />
        <HomeFeedItem
          postTitle="How I Learned to Code, and How You Can Too"
          volumeNumber="50"
          publishedDate="Aug 23 2017"
          postLink="/how-i-learned-to-code" />
        <HomeFeedItem
          postTitle="BeatsX Earphones Review"
          volumeNumber="49"
          publishedDate="Aug 17 2017"
          postLink="/beatsx-earphones-review" />
        <HomeFeedItem
          postTitle="Your Workflow is Not Important"
          volumeNumber="48"
          publishedDate="Aug 10 2017"
          postLink="/your-workflow-is-not-important" />
        <HomeFeedItem
          postTitle="5 Things I Learned Applying to Jobs in the Tech World"
          volumeNumber="47"
          publishedDate="Aug 3 2017"
          postLink="/5-things-i-learned-applying-to-jobs-in-tech" />
        <HomeFeedItem
          postTitle="Help People Help You"
          volumeNumber="46"
          publishedDate="Jul 27 2017"
          postLink="/help-people-help-you" />
        <HomeFeedItem
          postTitle="My Favorite Tech Purchases This Year"
          volumeNumber="45"
          publishedDate="Jul 20 2017"
          postLink="/favorite-tech-purchases-this-year" />
        <HomeFeedItem
          postTitle="How to Make Passive Social Media Traction"
          volumeNumber="44"
          publishedDate="Jul 13 2017"
          postLink="/how-to-make-passive-social-media-traction" />
        <HomeFeedItem
          postTitle="Why You Should Start Writing"
          volumeNumber="43"
          publishedDate="Jul 6 2017"
          postLink="/why-you-should-start-writing" />
        <HomeFeedItem
          postTitle="You Need to Work for Yourself More"
          volumeNumber="42"
          publishedDate="Jun 29 2017"
          postLink="/work-for-yourself-more" />
        <HomeFeedItem
          postTitle="Great Design, Brought to You by Caring"
          volumeNumber="41"
          publishedDate="Jun 21 2017"
          postLink="/great-design-by-caring" />
        <HomeFeedItem
          postTitle="Make Friends, Not Followers"
          volumeNumber="40"
          publishedDate="Jun 15 2017"
          postLink="/make-friends-not-followers" />
        <HomeFeedItem
          postTitle="Be Grateful, Your Work is Not Hard"
          volumeNumber="39"
          publishedDate="Jun 8 2017"
          postLink="/your-work-is-not-hard" />
        <HomeFeedItem
          postTitle="When to Seek Inspiration"
          volumeNumber="38"
          publishedDate="Jun 1 2017"
          postLink="/when-to-seek-inspiration" />
        <HomeFeedItem
          postTitle="Design and Develop Smaller"
          volumeNumber="37"
          publishedDate="May 25 2017"
          postLink="/design-and-develop-smaller" />
        <HomeFeedItem
          postTitle="The Best Thing I Learned as an Adult"
          volumeNumber="36"
          publishedDate="May 18 2017"
          postLink="/best-thing-learned-as-adult" />
        <HomeFeedItem
          postTitle="Stop Charging by the Hour"
          volumeNumber="35"
          publishedDate="May 11 2017"
          postLink="/stop-charging-hourly" />
        <HomeFeedItem
          postTitle="Subjective Versus Objective Design"
          volumeNumber="34"
          publishedDate="May 4 2017"
          postLink="/subjective-versus-objective-design" />
        <HomeFeedItem
          postTitle="Do You Need a Degree in Design?"
          volumeNumber="33"
          publishedDate="Apr 27 2017"
          postLink="/do-you-need-degree-design" />
        <HomeFeedItem
          postTitle="Disadvantages of Working Remote (and Their Fixes)"
          volumeNumber="32"
          publishedDate="Apr 20 2017"
          postLink="/disadvantages-of-working-remote" />
        <HomeFeedItem
          postTitle="Advantages of Working Remote"
          volumeNumber="31"
          publishedDate="Apr 13 2017"
          postLink="/advantages-of-working-remote" />
        <HomeFeedItem
          postTitle="Making Success Sustainable"
          volumeNumber="30"
          publishedDate="Apr 6 2017"
          postLink="/making-success-sustainable" />
        <HomeFeedItem
          postTitle="Apple LG UltraFine Monitor Review"
          volumeNumber="29"
          publishedDate="Mar 30 2017"
          postLink="/apple-lg-ultrafine-review" />
        <HomeFeedItem
          postTitle="How to Set Up Your Success Early"
          volumeNumber="28"
          publishedDate="Mar 23 2017"
          postLink="/set-up-your-success-early" />
        <HomeFeedItem
          postTitle="A Guide to a Healthy Work from Home Lifestyle"
          volumeNumber="27"
          publishedDate="Mar 16 2017"
          postLink="/guide-healthy-work-from-home-lifestyle" />
        <HomeFeedItem
          postTitle="How to Keep Learning and Stay Fulfilled"
          volumeNumber="26"
          publishedDate="Mar 9 2017"
          postLink="/keep-learning-stay-fullfilled" />
        <HomeFeedItem
          postTitle="Finding Comfort in Identity Crisis"
          volumeNumber="25"
          publishedDate="Mar 2 2017"
          postLink="/finding-comfort-identity-crisis" />
        <HomeFeedItem
          postTitle="Why You Should Give Away What You Know"
          volumeNumber="24"
          publishedDate="Feb 23 2017"
          postLink="/give-away-what-you-know" />
        <HomeFeedItem
          postTitle="How Sacrifices Provide Productivity"
          volumeNumber="23"
          publishedDate="Feb 16 2017"
          postLink="/how-sacrifices-provide-productivity" />
        <HomeFeedItem
          postTitle="Guidelines to Great Logo Design"
          volumeNumber="22"
          publishedDate="Feb 9 2017"
          postLink="/guidelines-great-logo-design" />
        <HomeFeedItem
          postTitle="Find Your Own Productivity Secret"
          volumeNumber="21"
          publishedDate="Feb 2 2017"
          postLink="/find-your-productivity-secret" />
        <HomeFeedItem
          postTitle="The Key to Growing Your Social Media Presence"
          volumeNumber="20"
          publishedDate="Jan 26 2017"
          postLink="/key-growing-social-media-presence" />
        <HomeFeedItem
          postTitle="How to Come Up with Side Project Ideas"
          volumeNumber="19"
          publishedDate="Jan 19 2017"
          postLink="/how-come-up-side-project-ideas" />
        <HomeFeedItem
          postTitle="Tips for Working Remote (Part 2)"
          volumeNumber="18"
          publishedDate="Jan 19 2017"
          postLink="/tips-for-working-remote-two" />
        <HomeFeedItem
          postTitle="Tips for Working Remote (Part 1)"
          volumeNumber="17"
          publishedDate="Jan 5 2017"
          postLink="/tips-for-working-remote-one" />
        <HomeFeedItem
          postTitle="My 2016"
          volumeNumber="16"
          publishedDate="Dec 29 2016"
          postLink="/2016" />
        <HomeFeedItem
          postTitle="Distractions Provide Progress"
          volumeNumber="15"
          publishedDate="Dec 22 2016"
          postLink="/distractions-provide-progress" />
        <HomeFeedItem
          postTitle="How to Cure Creative Block"
          volumeNumber="14"
          publishedDate="Dec 15 2016"
          postLink="/how-cure-creative-block" />
        <HomeFeedItem
          postTitle="People are More than Connections"
          volumeNumber="13"
          publishedDate="Dec 8 2016"
          postLink="/people-more-than-connections" />
        <HomeFeedItem
          postTitle="Less Perfection and More Iteration"
          volumeNumber="12"
          publishedDate="Dec 1 2016"
          postLink="/more-iteration-less-perfection" />
        <HomeFeedItem
          postTitle="Advantages of Side Projects"
          volumeNumber="11"
          publishedDate="Nov 24 2016"
          postLink="/advantages-of-side-projects" />
        <HomeFeedItem
          postTitle="Places to Help You Stay Inspired"
          volumeNumber="10"
          publishedDate="Nov 17 2016"
          postLink="/places-help-you-stay-inspired" />
        <HomeFeedItem
          postTitle="Why Vacations are Important"
          volumeNumber="9"
          publishedDate="Nov 10 2016"
          postLink="/why-vacations-are-important" />
        <HomeFeedItem
          postTitle="How to Get Better"
          volumeNumber="8"
          publishedDate="Nov 3 2016"
          postLink="/how-to-get-better" />
        <HomeFeedItem
          postTitle="How to Accept and Use Feedback"
          volumeNumber="7"
          publishedDate="Oct 27 2016"
          postLink="/accept-and-use-feedback" />
        <HomeFeedItem
          postTitle="Break Your Comfort Zone"
          volumeNumber="6"
          publishedDate="Oct 20 2016"
          postLink="/break-your-comfort-zone" />
        <HomeFeedItem
          postTitle="Budgeting Your Bandwidth"
          volumeNumber="5"
          publishedDate="Oct 13 2016"
          postLink="/budgeting-your-bandwidth" />
        <HomeFeedItem
          postTitle="Why You Should Seek Boredom"
          volumeNumber="4"
          publishedDate="Oct 6 2016"
          postLink="/why-to-seek-boredom" />
        <HomeFeedItem
          postTitle="The Advantages of Failing"
          volumeNumber="3"
          publishedDate="Sep 29 2016"
          postLink="/the-advantages-of-failing" />
        <HomeFeedItem
          postTitle="Trends Fade, Technique Does Not"
          volumeNumber="2"
          publishedDate="Sep 22 2016"
          postLink="/trends-fade-techniques-dont" />
        <HomeFeedItem
          postTitle="Introducing: The Rate of Change Blog"
          volumeNumber="1"
          publishedDate="Sep 15 2016"
          postLink="/the-rate-of-change" />
      </div>
    );
  }
}

export default HomeFeed;
